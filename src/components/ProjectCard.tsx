...
const ProjectCard: React.FC<Props> = ({title, description, link, image, rmm, status}) => {
    ...
    <div className="flex flex-col-reverse sm:flex-row-reverse sm:justify-end items-center">
        ...
        <a href={link} ><img src={image} alt=""  className="w-32 h-32 border-accent dark:border-dark-accent border-4"/></a>
    </div>
  
    <div className="order-first text-center sm:ml-8 text-sm mt-4 sm:mt-2">
        <span className="text-dark-text-primary">Status <br /><span className={status === "Active" ? "dark:text-green-600 text-green-400" : "text-red-600"}>{status}</span> </span>
    </div>
